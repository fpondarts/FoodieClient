...
class FavourOfferViewHolder(inflater: LayoutInflater, parent: ViewGroup):
    ...
    fun bind(item: OfferItem, listener: OnOfferItemClickListener){
        ...
        earnings.text = "${(item.earnings * 100.0 / 100.0).toInt().toString()} puntos"
        ...
        itemView.findViewById<Button>(R.id.button_detail).setOnClickListener(View.OnClickListener {
            listener.onFavourViewClick(item.offer_id,item.order_id,item.earnings.toInt())
        })
    }
